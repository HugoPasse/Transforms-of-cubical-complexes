...
void print_error(){
std::cout << "This program is made to benchmark the euler caracteristic transform algorithm.\n"
    <<"This program will create a few complexe and some vectors and compute the ect of the complexe in the vectors directions.\n"
		...
		if(argc > dimension + 2){
			...
			if(argc > dimension + 4){
				...
				for(int i=0; i<vect_count; i++){
	            	...
	            	cplx.compute_ect(vectors[i]);
	            	auto t3 = std::chrono::high_resolution_clock::now();
	            	tot += std::chrono::duration<double, std::milli>(t3-t2).count();
				}
